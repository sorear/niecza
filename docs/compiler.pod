...
The main entry points are in F<src/NieczaCompiler.pm6>.  The C<!compile>
method calls the parser (front end), a pipeline of conversion stages
(middle end) and code emitter (back end).

The parser is in F<src/NieczaFrontendSTD.pm6>, it uses F<NieczaGrammar.pm6>
and F<NieczaActions.pm6>.  The grammar uses F<src/STD.pm6>, a snapshot
...
other modules (Op, RxOp, Sig, CClass, OpHelpers, Operator) to create an
abstract syntax tree (AST) from Perl 6 source code.  The parser triggers
each action when it matches the corresponding token in the grammar.

=begin obsolete_docs

Content from here to the end of file is ignored by pod formatters.  We
agreed on 2011-08-30 that is is obsolete.  Delete each part as soon as
