...
=head3 ehspan($class,$name,$unused,$start,$end,$goto)
...
to be transferred to C<$end>.  C<$name> is used for targetted control
exceptions, possibly paired with the identity of the target frame.  Exactly
how return values are passed is still something of a mystery.
