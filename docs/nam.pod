...
The const op causes its argument to be evaluated once and stored in the constant table; it's semantically the same as the identity.
...
Evaluates arguments in sequens and returns Nil (taken from CORE?).
