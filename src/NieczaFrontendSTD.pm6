...
method parse(:$unitname, :$filename, :$source, :$outer, :$run, :$main, :$evalmode, :$repl) {
    ...
    my $compart = $*backend.push_compartment unless $evalmode;
    LEAVE { $*backend.pop_compartment($compart) unless $evalmode };
    my $*unit = $*backend.create_unit($outer // $compart, $unitname, $filename, $*no_incl_source ?? $Backend.gethash($source) !! $source, $main, $run);
