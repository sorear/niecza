...
C<RUN_CLR=mono-sgen> in this case.
...
or .NET 4.0: C<http://goo.gl/PMKvB>

Currently, you need to make a change to the C<Makefile> to enable
a successful build under Cygwin, but once sorear makes a C<Configure.pl> and/or
uses the mono autotools templates, this won't be needed... merely remove C<mono>
from the line starting with C<RUN_CLR=>.  Niecza also runs on mono/Windows.
